...
def main(config):
	...
	for name, test_set in testing_data_loader.items():
		...
		for i, batch in enumerate(test_set):
			...
			logger.log('[ Image: {:s}  PSNR: {:.4f} SSIM: {:.4f} Inference Time: {:.4f}]'.format(img_name, psnr, ssim, infer_time))
			avg_psnr += psnr
			avg_ssim += ssim
		avg_psnr = avg_psnr / idx
		avg_ssim = avg_ssim / idx
		valid_t = time.time() - valid_start_time
		logger.log('[ Set: {:s} Time:{:.3f}] PSNR: {:.4f} SSIM {:.4f}'.format(name, valid_t, avg_psnr, avg_ssim))
	...
	logger.log('[ Total Average of Sets: PSNR: {:.4f} SSIM {:.4f}'.format(total_avg_psnr, total_avg_ssim))
